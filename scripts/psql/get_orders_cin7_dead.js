...
async function getOrdersCin7Dead() {
	...
	await uploadToSheet(formattedData);
}
...
async function uploadToSheet(data) {
	const cin7Inserter = sheetInserter({
		outSheetID: "17-RXYMPeujucrW3jk-gWKAWnVLAFVtj-rAvJdwcKP4s",
		outSheetName: "orders",
		outSheetRange: "A1",
	});

	await cin7Inserter.run(data);
}
